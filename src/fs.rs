    ...
    match unsafe{HANDLES.get_mut(&handle)} {
        Some(open_file) => {
            if open_file.position + bytes.len() <= open_file.data.len() {
                ...
                open_file.data.get_mut(open_file.position..last_position).unwrap().clone_from_slice(bytes);
            } else if open_file.position > open_file.data.len() {
                open_file.data.append(&mut [0].repeat(open_file.position-open_file.data.len()));
                open_file.data.append(&mut bytes.to_vec().clone());
            } else {
                let last_position = open_file.position + bytes.len();
                let middle_position = open_file.data.len()-open_file.position;
                open_file.data.get_mut(open_file.position..last_position).unwrap().clone_from_slice(bytes.get(0..middle_position).unwrap());
                open_file.data.append(&mut bytes.get(middle_position..bytes.len()).unwrap().to_vec().clone());
            }
    ...
    match unsafe{HANDLES.get_mut(&handle)} {
        Some(open_file) => {
            if open_file.position + bytes.len() > open_file.data.len() {
                ...
                bytes.get_mut(0..length).unwrap().clone_from_slice(open_file.data.get_mut(open_file.position..data_len).unwrap());
                ...
                let last_position = open_file.position + bytes.len();
                bytes.clone_from_slice(open_file.data.get_mut(open_file.position..last_position).unwrap());
                open_file.position += bytes.len();
                bytes.len() as isize
            }
        }
